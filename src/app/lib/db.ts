...
export async function checkSymbol(symbol: string){ // 確認済み
...
export async function SimulateSmashday(param: {symbol: string, start: string, end: string, tradeType: "buy" | "sell" | "both", HLBand: number | null, EMA: {short: number, long: number} | null}){
    ...
    let result: {sumAll: number,
                 sumGain: number,
                 sumLoss: number,
                 cntGain: number,
                 cntLoss: number,
                 details: {startDate: string,
                           endDate: string,
                           tradeType: "Buy" | "Sell" | "",
                           outcome: "Loss" | "Gain" | "",
                           amount: string}[]}
    ...
    let tradePrice = 0, tradeType: "Buy" | "Sell" | "" = "", borderHigh = 0, borderLow = 0, highestClose = 0, lowestClose = 0, startDate = "";
    ...
    for(let i = 3; i < prices.length; i++){
        if(tradeType != ""){
            let outcome: "Loss" | "Gain" | "" = "", amount = 0;
            if(tradeType == "Buy"){
                // Loss
            ...
            if(useBuy){
                let signalBuy = prices[i - 3].close > prices[i - 2].close && prices[i - 2].low > prices[i - 1].close && prices[i - 1].high < prices[i].high;

                const emashort = prices[i].emashort, emalong = prices[i].emalong;
                if(emashort && emalong){ // undefinedでないことの確認
            ...
            if(useSell){
                let signalSell = prices[i - 3].close < prices[i - 2].close && prices[i - 2].low < prices[i - 1].close && prices[i - 1].low > prices[i].low;
...
function addIndicator(rawPrices: {date: string, close: number, high: number, low: number}[], param: {HLBand: number | null, EMA: {short: number, long: number} | null}){
    let result: {date: string, close: number, high: number, low: number, hband?: number, lband?: number, emashort?: number, emalong?: number}[] = [];
    ...
    let spanEMAShort = 0, spanEMALong = 0, weightShort = 0, weightLong = 0;
    if(param.EMA != null){
    ...
    for(let i = 0; i < rawPrices.length; i++){
        let price: {date: string, close: number, high: number, low: number, hband?: number, lband?: number, emashort?: number, emalong?: number} = rawPrices[i];
        let hband = -1, lband = 1000000;
...
function convertDate_num(date: string){
    ...
    return (Number(devided_date[0]) - 2000) * 366 + (Number(devided_date[1]) - 1) * 31 + Number(devided_date[2]) - 1;
}